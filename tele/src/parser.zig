...
fn parse_tokens(token_queue: *TokenQueue, allocator: std.mem.Allocator) ParserError!std.ArrayList(*TeleAst) {
    ...
    while (!token_queue.empty()) {
        ...
        if (is_float(node.*.body)) {
        ...
        } else if (is_arrow_operator(node.*.body)) {
            if (list.items.len == 0) {
            ...
            if (ast1.*.ast_type != TeleAstType.paren_exp) {
                // TODO: Error expected paren_exp
                return ParserError.ParsingFailure;
            }
            // Free arrow operator body
            allocator.free(node.*.body);
            const t = parse_arrow_operator(token_queue, allocator, ast1) catch {
                return ParserError.ParsingFailure;
            };
            errdefer tele_ast.free_tele_ast(t, allocator);
            list.append(t) catch {
            ...
            allocator.free(node.*.body);
            const t = parse_paren_exp(token_queue, allocator, node.*.col) catch {
                return ParserError.ParsingFailure;
            };
            errdefer tele_ast.free_tele_ast(t, allocator);
            list.append(t) catch {
            ...
            } else if (is_type_def(node.*.body)) {
                allocator.free(node.*.body);
                const t = parse_type_definition(token_queue, allocator, node.*.col) catch {
                    return ParserError.ParsingFailure;
                };
                errdefer tele_ast.free_tele_ast(t, allocator);
                list.append(t) catch {
            ...
            } else if (is_record_def(node.*.body)) {
                allocator.free(node.*.body);
                const t = parse_record_definition(token_queue, allocator, node.*.col) catch {
                    return ParserError.ParsingFailure;
                };
                errdefer tele_ast.free_tele_ast(t, allocator);
                list.append(t) catch {
...
fn parse_paren_exp(token_queue: *TokenQueue, allocator: std.mem.Allocator, col: usize) !*TeleAst {
...
fn parse_arrow_operator(token_queue: *TokenQueue, allocator: std.mem.Allocator, paren_exp: *TeleAst) !*TeleAst {
    var buffer_token_queue = try TokenQueue.init(allocator);
    errdefer buffer_token_queue.deinit();
    if (token_queue.empty()) {
        return ParserError.ParsingFailure;
...
fn parse_type_definition(token_queue: *TokenQueue, allocator: std.mem.Allocator, current_col: usize) !*TeleAst {
    const node3 = token_queue.pop() catch {
        return ParserError.ParsingFailure;
    };
...
fn parse_record_definition(token_queue: *TokenQueue, allocator: std.mem.Allocator, current_col: usize) !*TeleAst {
    const node3 = token_queue.pop() catch {
        return ParserError.ParsingFailure;
    };
